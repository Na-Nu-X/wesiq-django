...
export async function addComment(article_id:number, write_comment_form:HTMLDivElement, all_comments:HTMLDivElement, comment_forum:HTMLDivElement, parent_id:number|null, comments_counter:HTMLParagraphElement, logged_in_user_role:"developer"|"admin"|"user"|"unauthorized"):Promise<void> {
    try {
        ...
        const add_article_comment_response:addCommentResponse = await sendPOST(window.location.pathname, comment_data, "add-article-comment") // Sends The Data With POST

        // If The Response Isn't Success
        if(!add_article_comment_response.success) {
            displayMessage(add_article_comment_response.message, "error") // Displays The Error Message
            return
        }
        ...
        one_comment_container.dataset["comment_id"] = String(add_article_comment_response.comment.id) // Stores The Comment ID
        ...
        comment_author_profile_picture_link.href = interpolate(gettext("/sk/profil/%s"), [add_article_comment_response.comment.user.username]) // Sets The Link To The User's Profile
        ...
        if(parent_id) {
            ...
            (write_comment_form.querySelector(".comment") as HTMLTextAreaElement).placeholder = gettext("Napísať komentár")
            ...
            if(parent_comment) {
                ...
                if(add_article_comment_response.comment.level === 2) {
        ...
        if(add_article_comment_response.comment.level < 5) {
        ...
        date_paragraph.textContent = getFormattedDate(add_article_comment_response.comment.creation_time)
...
export function toggleReplyOnComment(icon:HTMLElement, write_comment_form:HTMLDivElement, one_comment:HTMLDivElement):void {
    ...
    if(icon.classList.contains("fa-comment")) {
        if(!write_comment_form.dataset["parent_id"] || write_comment_form.dataset["parent_id"].trim() === "") {
            ...
            (write_comment_form.querySelector(".comment") as HTMLTextAreaElement).placeholder = interpolate(gettext("Odpoveď užívateľovi %s"), [(one_comment.querySelector(".comment_container .user .username") as HTMLParagraphElement).textContent]) // Sets The Placeholder
        }
    }

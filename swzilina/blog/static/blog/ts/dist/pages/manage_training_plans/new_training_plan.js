...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("dragover", function (event) {
        if (event.target instanceof Node && dragged_exercise && !training_plan.contains(event.target))
            event.preventDefault(); // If There Is Dragged Exercise And Dragover Element Isn't Inside The Training Plan (Makes The Drop Zone Functional)
    });
    document.addEventListener("drop", function (event) {
        if (event.target instanceof Node && dragged_exercise && !training_plan.contains(event.target))
            removeExercise(dragged_exercise, training_plan, new_training_plan_state); // Removes Dragged Exercise From The Training Plan (If There Is Dragged Exercise And Drop Element Isn't Inside The Training Plan)
    });
    ...
    training_plan.addEventListener("drop", function (event) {
        if (event.target === drop_zone)
            addExercise(training_plan, new_training_plan_state); // Drop Zone For The First Exercise (Adds Dragged Exercise From Exercise Selection To The Training Plan)
        if (event.target === training_plan.querySelectorAll(".exercise")[new_training_plan_state.active_exercise_index])
            addExercise(training_plan, new_training_plan_state); // Drop Zone On Active Exercise In The Training Plan (Adds Dragged Exercise From Exercise Selection To The Training Plan) 
        if (global_state.selection_dragged_exercise)
    ...
    training_plan.addEventListener("wheel", function (event) {
        ...
        if (event.target !== drop_zone) {
            if (event.deltaY < 0)
                changeExercises(new_training_plan_state.active_exercise_index - 1, training_plan, new_training_plan_state); // Changes Training Plan Exercises (Shows Next Exercise)
            if (event.deltaY > 0)
                changeExercises(new_training_plan_state.active_exercise_index + 1, training_plan, new_training_plan_state); // Changes Training Plan Exercises (Shows Previous Exercise)
        }
    });
    ...
    new_training_plan.addEventListener("mouseover", function (event) {
        if (!(event.target instanceof Node) || event.target.classList.contains("bar_container") || event.target.parentNode.classList.contains("bar_container"))
            global_state.hovered_element = "new_training_plan_exercises_bars"; // Sets Hovered Element For Bar Container
    });
...
//# sourceMappingURL=new_training_plan.js.map
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function trainingPlanProgress() {
        ...
        let current_set = parseInt(set_progress[0].textContent) || 1; // Gets Current Set Value
        const sets_amount = parseInt(set_progress[1].textContent) || 1; // Gets Total Amount Of Sets Of The Active Exercise
    ...
    function startActivity() {
        ...
        activity_timer_interval = setInterval(function () {
            ...
            if (xp_boost_remaining_time <= 0) {
                ...
                current_activity_info.textContent = `${xp_boost}x XP`; // If There Is An Active XP Boost
            if (!exercises)
                return;
            ...
            if (exercises[active_exercise_index].classList.contains("active")) {
                ...
                if (existing_exercise)
                    existing_exercise.elapsed_time = exercises_elapsed_time; // Updated Only Elapsed Time For Existing Exercise
    ...
    function stopActivity() {
        ...
        if (activity_timer_elapsed_time >= 1) {
            ...
            if (exercises && exercises[0] && exercises_summary.length > 0)
                new_activity_data.type = exercises[0].dataset.type || null; // Sends Type Of Training If Activity Was Started With Training Plan
...
//# sourceMappingURL=training_session.js.map
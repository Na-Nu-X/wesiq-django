import { global_state, new_training_plan_state, edit_training_plan_state, exercise_selection_state } from "./state.js";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function changeWeight(exercise, operation) {
        const current_weight = exercise.dataset.weight || "0"; // Gets Current Added Or Subtracted Weight
        ...
        if (exercise.dataset.requires_weight == "True" && operation === "decrease" && current_weight_number <= 0)
            return; // Can't Get Negative Number If The Exercise Has Required Weight
    ...
    selection_exercises.forEach(function (one_exercise) {
        ...
        one_exercise.addEventListener("dblclick", function (event) {
            ...
            if ((event.target instanceof Node) && !event.target.classList.contains("increase_weight") && !event.target.classList.contains("decrease_weight") && !event.target.parentNode.classList.contains("weight")) {
                global_state.selection_dragged_exercise = one_exercise; // Sets Selection Dragged Exercise
                // Adds Exercise To The New Training Plan
                if (global_state.focused_element === "new_training_plan")
                    addExercise(new_training_plan.querySelector(".training_plan"), new_training_plan_state); // Adds Dragged Exercise From Exercise Selection To The Training Plan
                // Adds Exercise To The Edit Training Plan
                if (global_state.focused_element === "edit_training_plan")
                    addExercise(edit_training_plan.querySelector(".training_plan"), edit_training_plan_state); // Adds Dragged Exercise From Exercise Selection To The Training Plan
                global_state.selection_dragged_exercise = null; // Deletes Selection Dragged Exercise
            }
        });
        ...
        one_exercise.addEventListener("wheel", function (event) {
            ...
            if (event.deltaY < 0) {
                if (!one_exercise.classList.contains("custom_exercise") && !one_exercise.classList.contains("warm_up"))
                    changeWeight(one_exercise, "increase"); // Increases Weight (Except Of Custom Exercise And Warm Up)
            }
            if (event.deltaY > 0) {
                if (!one_exercise.classList.contains("custom_exercise") && !one_exercise.classList.contains("warm_up"))
                    changeWeight(one_exercise, "decrease"); // Increases Weight (Except Of Custom Exercise And Warm Up)
            }
        });
        // Sets And Deletes Hovered Exercise Selection Exercise Element
        one_exercise.addEventListener("mouseover", function (event) {
            if (!event.target.closest(".exercise").classList.contains("custom_exercise") && !event.target.closest(".exercise").classList.contains("warm_up"))
                exercise_selection_state.hovered_exercise = event.target.closest(".exercise"); // Sets Hovered Exercise Selection Exercise Element (Except Of Custom Exercise And Warm Up)
        });
    ...
    document.addEventListener("keydown", function (event) {
        if (exercise_selection_state.hovered_exercise) {
            ...
            if (event.key === "ArrowUp")
                changeWeight(exercise_selection_state.hovered_exercise, "increase"); // Increases Weight
            else if (event.key === "ArrowDown")
                changeWeight(exercise_selection_state.hovered_exercise, "decrease"); // Decreases Weight
        }
    });
});
//# sourceMappingURL=exercise_selection.js.map
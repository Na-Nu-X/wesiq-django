import { getMinimalistFormattedTime, getElapsedSeconds } from "../../../utils/timer.js";
...
export function changeSets(button, operation) {
    ...
    if (sets_number < 1 || sets_number > 100)
        return; // Do Nothing
...
export function getPeriods(exercise) {
    ...
    for (let i = 0; i < sets_inputs_values.length; i++) {
        const reps_number = reps_inputs_values[i];
        const sets_number = sets_inputs_values[i];
        if (!sets_number || !reps_number)
            continue;
        for (let j = 0; j < sets_number; j++) {
            periods.unshift(reps_number); // Saves Values To Periods
        }
    }
    if (periods.length === 0)
        periods.push(getElapsedSeconds(exercise.querySelector(".timer_container .timer h3").textContent)); // Elapsed Seconds Of The Warm Up Timer

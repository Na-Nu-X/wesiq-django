...
export function saveTrainingPlan(container, state) {
    ...
    if (training_plan_title.value !== "" && exercises.length !== 0 && custom_exercises_without_name.length === 0) {
        ...
        let action = "new_training_plan"; // Stores Action Value
        if (container.classList.contains("new_training_plan"))
            action = "new_training_plan";
        else if (container.classList.contains("edit_training_plan"))
            action = "edited_training_plan";
        ...
        exercises.forEach(function (one_exercise) {
            ...
            const unit = one_exercise.dataset.unit ?? "reps"; // Gets Exercise Unit Type (Reps Or Seconds)
        ...
        if (action === "new_training_plan")
            sendNotification(`Tréningový plán ${training_plan_title.value} bol úspešne pridaný.`);
        if (action === "edited_training_plan")
            sendNotification(`Tréningový plán ${training_plan_title.value} bol úspešne upravený.`);
        location.reload(); // Reloads The Page
    }
}
//# sourceMappingURL=saveTrainingPlan.js.map
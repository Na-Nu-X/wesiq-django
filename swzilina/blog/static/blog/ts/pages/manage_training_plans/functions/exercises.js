...
function isExistingExercise(training_plan) {
    ...
    return [...exercises].some(function (one_exercise) {
        ...
        const selection_dragged_exercise_weight = Number(global_state.selection_dragged_exercise.dataset.weight || 0); // Gets Current Added Or Subtracted Weight Of The Dragged Exercise
        return (title === selection_dragged_exercise_name && selection_dragged_exercise_weight === 0 || // For Example: Front Lever
...
function createExerciseTitle(exercise_name, exercise_weight) {
    if (global_state.selection_dragged_exercise.classList.contains("custom_exercise"))
        return undefined; // Skips Custom Exercise
    if (global_state.selection_dragged_exercise.classList.contains("warm_up"))
        return undefined; // Skips Warm Up
...
export function addExercise(training_plan, state) {
    ...
    if (global_state.selection_dragged_exercise && !isExistingExercise(training_plan) && !global_state.selection_dragged_exercise.classList.contains("warm_up") || global_state.selection_dragged_exercise.classList.contains("custom_exercise")) {
        ...
        if (global_state.selection_dragged_exercise.dataset.unit === "reps")
            exercise_template_clone.querySelector(".labels .unit_amount").textContent = "Počet opakovaní";
        if (global_state.selection_dragged_exercise.dataset.unit === "seconds")
            exercise_template_clone.querySelector(".labels .unit_amount").textContent = "Počet sekúnd";
        if (global_state.selection_dragged_exercise.dataset.unit === "steps")
            exercise_template_clone.querySelector(".labels .unit_amount").textContent = "Počet krokov";
        ...
        if (global_state.selection_dragged_exercise.classList.contains("custom_exercise"))
            addCustomExercise(exercise_template_clone.querySelector(".exercise")); // Adds Custom Exercise To The Training Plan
...
export function changeSlides(training_plan, state, show_first = false) {
    ...
    if (drop_zone.classList.contains("active"))
        drop_zone.classList.remove("active"); // Hides Training Plan Drop Zone On First Exercise Change
    else {
        if (show_first) {
...
export function changeExercisePosition(dropped_bar_index, dragged_bar, training_plan, state) {
    ...
    if (dragged_bar) {
        ...
        const exercises = training_plan.querySelectorAll(".exercise"); // Gets All Training Plan Exercises
        const active_exercise = exercises[state.active_exercise_index]; // Gets The Active Exercise
        const dragged_exercise = exercises[dragged_bar_index]; // Gets The Dragged Exercise
        const dropped_exercise = exercises[dropped_bar_index]; // Gets The Dropped Exercise
        if (dragged_exercise.querySelector(".title").textContent === "Warm Up" || dropped_exercise.querySelector(".title").textContent === "Warm Up")
            return; // Do Nothing If Exercise Of Dropped Bar Index Or Dragged Bar Index Is Warm Up

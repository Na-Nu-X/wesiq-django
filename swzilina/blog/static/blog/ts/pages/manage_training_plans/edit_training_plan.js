...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function generateTrainingPlan(exercises_data, container) {
        ...
        // Day
        let training_plan_day = training_plan_days_order[edit_training_plan_state.active_training_plan_index] ?? null; // Gets The First Value From Training Plan Days Order (Current Or Upcoming Day By Default)
        ...
        ordered_exercises_data.forEach(function (one_exercise_data) {
            ...
            const periods_data = JSON.parse(one_exercise_data.dataset.periods || "[0]"); // Gets Exercise Sets & Reps Periods
            const unit_data = one_exercise_data.dataset.unit || "reps"; // Gets Exercise Unit Type (Reps Or Seconds)
            ...
            if (day_data !== null) {
                ...
                if (training_plan_day === day_data) {
                    ...
                    // Creates Exercises
                        ...
                        if (unit_data === "reps")
                            exercise_template_clone.querySelector(".exercise .labels .unit_amount").textContent = "Počet opakovaní";
                        if (unit_data === "seconds")
                            exercise_template_clone.querySelector(".exercise .labels .unit_amount").textContent = "Počet sekúnd";
                        if (unit_data === "steps")
                            exercise_template_clone.querySelector(".exercise .labels .unit_amount").textContent = "Počet krokov";
    ...
    edit_training_plan.addEventListener("click", function (event) {
        // Training Plan Bars
        if (event.target instanceof Node && event.target.classList.contains("bar") && event.target.parentNode.classList.contains("training_plan_bar_container")) {
    ...
    document.addEventListener("dragover", function (event) {
        if (event.target instanceof Node && dragged_exercise && !training_plan.contains(event.target))
            event.preventDefault(); // Makes The Drop Zone Functional (If There Is Dragged Exercise And Dragover Element Isn't Inside The Training Plan)
    });
    document.addEventListener("drop", function (event) {
        if (event.target instanceof Node && dragged_exercise && !training_plan.contains(event.target))
            removeExercise(dragged_exercise, training_plan, edit_training_plan_state); // Removes Dragged Exercise From The Training Plan (If There Is Dragged Exercise And Drop Element Isn't Inside The Training Plan)
    });
    ...
    training_plan.addEventListener("dragover", function (event) {
        if (event.target === drop_zone)
            event.preventDefault(); // Drop Zone For The First Exercise (Makes The Drop Zone Functional)
        if (event.target === training_plan.querySelectorAll(".exercise")[edit_training_plan_state.active_exercise_index])
            event.preventDefault(); // Drop Zone On Active Exercise In The Training Plan (Makes The Drop Zone Functional)
        if (global_state.selection_dragged_exercise)
            training_plan.classList.add("animate"); // Adds Drag Animation (Executes Only If The Dragged Element Is Selection Dragged Exercise)
        if (event.target.classList.contains("bar"))
            event.preventDefault(); // Drop Zone On Bars In The Bar Container (Makes The Drop Zone Functional)
    });
    training_plan.addEventListener("drop", function (event) {
        if (event.target === drop_zone)
            addExercise(training_plan, edit_training_plan_state); // Drop Zone For The First Exercise (Adds Dragged Exercise From Exercise Selection To The Training Plan)
        if (event.target === training_plan.querySelectorAll(".exercise")[edit_training_plan_state.active_exercise_index])
            addExercise(training_plan, edit_training_plan_state); // Drop Zone On Active Exercise In The Training Plan (Adds Dragged Exercise From Exercise Selection To The Training Plan) 
        if (global_state.selection_dragged_exercise)
            training_plan.classList.remove("animate"); // Removes Drag Animation (Executes Only If The Dragged Element Is Selection Dragged Exercise)
        ...
        if (event.target.classList.contains("bar")) {
            ...
            if (!(event.target instanceof Node) || !event.target.parentNode || !dragged_bar)
                return; // Catch Errors
    ...
    training_plan.addEventListener("click", function (event) {
        ...
        if (event.target.classList.contains("bar")) {
            if (!(event.target instanceof Node) || !event.target.parentNode)
                return; // Catch Errors
            const clicked_bar_index = [...event.target.parentNode.querySelectorAll(".bar")].indexOf(event.target); // Gets Index Of The Clicked Bar
        ...
        if (event.target.closest(".unit_select_menu")) {
            ...
            if (event.target.closest(".option")) {
                ...
                if (clicked_option.dataset.unit_option)
        ...
        if (!(event.target instanceof Node) || event.target.classList.contains("subtract_time") || event.target.parentNode.classList.contains("subtract_time")) {
        ...
        if (!(event.target instanceof Node) || event.target.classList.contains("add_time") || event.target.parentNode.classList.contains("add_time")) {
            const warm_up = event.target.closest(".exercise"); // Gets Warm Up From The Training Plan
    ...
    edit_training_plan.addEventListener("wheel", function (event) {
        if (!(event.target instanceof Node) || event.target.classList.contains("training_plan_bar_container") || event.target.parentNode.classList.contains("training_plan_bar_container")) {
            event.preventDefault(); // Stop Scrolling
            if (event.deltaY < 0)
                changeTrainingPlans(edit_training_plan_state.active_training_plan_index + 1); // Changes Training Plans (Shows Next Training Plan)
            if (event.deltaY > 0)
                changeTrainingPlans(edit_training_plan_state.active_training_plan_index - 1); // Changes Training Plans (Shows Previous Training Plan)
        }
    });
    ...
    training_plan.addEventListener("wheel", function (event) {
        ...
        if (event.target !== drop_zone) {
            if (event.deltaY < 0)
                changeExercises(edit_training_plan_state.active_exercise_index - 1, training_plan, edit_training_plan_state); // Changes Training Plan Exercises (Shows Next Exercise)
            if (event.deltaY > 0)
                changeExercises(edit_training_plan_state.active_exercise_index + 1, training_plan, edit_training_plan_state); // Changes Training Plan Exercises (Shows Previous Exercise)
        }
    });
    ...
    edit_training_plan.addEventListener("mouseover", function (event) {
        if (!(event.target instanceof Node) || event.target.classList.contains("bar_container") || event.target.parentNode.classList.contains("bar_container"))
            global_state.hovered_element = "edit_training_plan_exercises_bars"; // Sets Hovered Element For Bar Container
        else if (!(event.target instanceof Node) || event.target.classList.contains("training_plan_bar_container") || event.target.parentNode.classList.contains("training_plan_bar_container"))
            global_state.hovered_element = "edit_training_plan_bars"; // Sets Hovered Element For Training Plan Bar Container
    });
...
//# sourceMappingURL=edit_training_plan.js.map